...
AST_node_t* generate_AST(map_t* worddefs,char** toStr,char* triggerword){
  ...
  token_t* token=get_next_token(toStr);
  while(strcmp(token->str,breakword)){
    if(token->type!=4){ // or default word
      ...
      if(map_containsKey(worddefs,token->str)){ // user defined word (already defined)
        AST_node_t* sub=map_get(worddefs,token->str);
